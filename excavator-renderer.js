...
let originalAnchors = [];
let groundLine = { start: { x: 0, y: 0 }, end: { x: 0, y: 0 } };
let originalSizes = {}; // Stores original SIZE from config (assumed half-real)
let originalScales = {}; // Stores original SCALE from config (for reset)
      ...
      <div class="excavator-layout">
        ...
        <div class="excavator-controls">
          ...
          <fieldset class="control-section">
            ...
            <div class="form-group"> <label for="real-body-height" title="挖机旋转中心到地面的垂直距离">(信息)车身高:</label> <input type="number" id="real-body-height" placeholder="例如: 5230" /> </div>
            <div class="form-group"> <label for="real-body-boom" title="车身转轴到大臂转轴距离">车身-大臂:</label> <input type="number" id="real-body-boom" placeholder="例如: 230" /> </div>
            ...
            <div class="form-group"> <label for="real-bucket-tip" title="铲斗转轴到铲斗尖距离">铲斗轴-尖:</label> <input type="number" id="real-bucket-tip" placeholder="例如: 2235" /> </div>
            ...
            <div id="scale-ratios-display" class="result-area" style="font-size: 0.9em; line-height: 1.4;"> 应用比例后将在此显示各部件最终缩放因子 (相对Config尺寸)。 </div>
          </fieldset>
...
async function initExcavatorRenderer() {
    try {
        ...
        originalConfigData.forEach(partConfig => {
            ...
            originalSizes[partName] = [...partConfig.size]; // Store original config size (half-real)
        ...
        await Promise.all( excavatorParts.map(async (partConfig) => { const img = new Image(); img.src = `./assets/${partConfig.part}.png`; return new Promise((resolve, reject) => { img.onload = () => { excavatorImages[partConfig.part] = img; resolve(); }; img.onerror = (err) => { console.error(`Error loading image for ${partConfig.part}:`, err); reject(err); }; }); }) );
        setupExcavator();
        setupControlEvents();
        updateExcavatorAngles();

      } catch (error) {
        console.error("初始化挖掘机渲染器时出错:", error); const pageContent = document.getElementById('page-content'); if (pageContent) { pageContent.innerHTML = `<div class="card error"><p>加载挖掘机配置或图像失败: ${error.message}。</p></div>`; }
      }
}
...
function calculatePartDistanceUsingSize(rotateAnchor, nextAnchor, size) {
    if (!rotateAnchor || !nextAnchor || !size || size.length < 2) return 0; const dx_local = nextAnchor[0] - rotateAnchor[0]; const dy_local = nextAnchor[1] - rotateAnchor[1]; const dx_scaled = dx_local * size[0]; const dy_scaled = dy_local * size[1]; if (isNaN(dx_scaled) || isNaN(dy_scaled)) return 0; return Math.sqrt(dx_scaled * dx_scaled + dy_scaled * dy_scaled);
}
...
function updateAnchorInputs() {
    const selectedPartName = document.getElementById("anchor-part")?.value; const rotateXInput = document.getElementById("rotate-anchor-x"); const rotateYInput = document.getElementById("rotate-anchor-y"); const nextXInput = document.getElementById("next-anchor-x"); const nextYInput = document.getElementById("next-anchor-y"); if (!selectedPartName || !rotateXInput || !rotateYInput || !nextXInput || !nextYInput || !excavatorParts) return; const part = excavatorParts.find(p => p.part === selectedPartName); if (part && part.rotate_anchor && part.next_anchor) { rotateXInput.value = part.rotate_anchor[0]?.toFixed(3) ?? 0; rotateYInput.value = part.rotate_anchor[1]?.toFixed(3) ?? 0; nextXInput.value = part.next_anchor[0]?.toFixed(3) ?? 0; nextYInput.value = part.next_anchor[1]?.toFixed(3) ?? 0; } else { rotateXInput.value = ''; rotateYInput.value = ''; nextXInput.value = ''; nextYInput.value = ''; }
}
...
/**
 * Resets part sizes to their initial values (original config size * original config scale).
 */
function resetSizesToOriginal() {
    ...
    if(changed){
        ...
        renderExcavator();
         ...
         const displayDiv = document.getElementById('scale-ratios-display');
         if (displayDiv) { displayDiv.innerHTML = '部件尺寸已是初始值。应用比例后将在此显示各部件缩放比。'; }
    }
}

/**
 * Applies scaling based on real dimensions, keeping Body size fixed at its original config size,
 * and adjusting other parts proportionally based on real dimension ratios relative to Body->Boom segment.
 */
function applyRealDimensionsScale() {
    console.log("--- Running applyRealDimensionsScale ---"); // Add entry log
    ...
    const realBodyHeightRef = parseFloat(document.getElementById('real-body-height')?.value); // Read for table display context only

    console.log("Inputs Read:", { realBodyToBoomDist, realBoomToArmDist, realArmToBucketDist, realBucketToTipDist, realBodyHeightRef });
    ...
    try {
        console.group("Scaling Calculations"); // Group console logs
        ...
        console.log(`Real Ratios (Rel BodyBoom): BoomArm=${realRatio_BoomArm_To_BodyBoom.toFixed(3)}, ArmBucket=${realRatio_ArmBucket_To_BodyBoom.toFixed(3)}, BucketTip=${realRatio_BucketTip_To_BodyBoom.toFixed(3)}`);

        // 4. Get Original Config Part data and Calculate Original Pixel Distances
        //    using ORIGINAL config sizes (half-real) and ORIGINAL anchors
        ...
        if (!origBodyConf || !origBoomConf || !origArmConf || !origBucketConf){ alert("无法找到所有部件的原始配置信息。"); console.groupEnd(); return; }

        // Calculate distances based on ORIGINAL config sizes (values stored in originalSizes)
        ...
        console.log(`Original Px Distances (using config sizes): BodyBoom=${originalPxBodyBoom.toFixed(1)}, BoomArm=${originalPxBoomArm.toFixed(1)}, ArmBucket=${originalPxArmBucket.toFixed(1)}, BucketTip=${originalPxBucketTip.toFixed(1)}`);

        if (originalPxBodyBoom <= 1e-6) { alert("原始配置中车身->大臂像素距离过小或为零，无法计算比例。"); console.groupEnd(); return; }

        // 5. Determine Target Rendered Distances based on Body original config size
        const targetPxBodyBoom = originalPxBodyBoom; // Base pixel distance uses original config size's result
        ...
        console.log(`Target Px Distances: BodyBoom=${targetPxBodyBoom.toFixed(1)}, BoomArm=${targetPxBoomArm.toFixed(1)}, ArmBucket=${targetPxArmBucket.toFixed(1)}, BucketTip=${targetPxBucketTip.toFixed(1)}`);

        // 6. Calculate Required Scaling Factors (relative to original config sizes)
        // This factor tells us by how much to scale the original config size
        const bodyScaleFactor = 1.0; // Body keeps its original config size
        const boomScaleFactor = (originalPxBoomArm > 1e-6) ? targetPxBoomArm / originalPxBoomArm : 1;
        const armScaleFactor = (originalPxArmBucket > 1e-6) ? targetPxArmBucket / originalPxArmBucket : 1;
        const bucketScaleFactor = (originalPxBucketTip > 1e-6) ? targetPxBucketTip / originalPxBucketTip : 1;
        console.log(`Calculated Scale Factors (Rel Config Size): Body=${bodyScaleFactor.toFixed(3)}, Boom=${boomScaleFactor.toFixed(3)}, Arm=${armScaleFactor.toFixed(3)}, Bucket=${bucketScaleFactor.toFixed(3)}`);

        // 7. Apply Scaling (multiply original config size by the scale factor)
        let scalingApplied = false;
        excavatorParts.forEach(part => {
            const partName = part.part;
            const origSize = originalSizes[partName]; // Get original config size (half-real)
            if (!origSize) return;

            let scaleFactor = 1.0;
            if (partName === 'body') scaleFactor = bodyScaleFactor;
            else if (partName === 'boom') scaleFactor = boomScaleFactor;
            else if (partName === 'arm') scaleFactor = armScaleFactor;
            else if (partName === 'bucket') scaleFactor = bucketScaleFactor;

            const newWidth = origSize[0] * scaleFactor;
            const newHeight = origSize[1] * scaleFactor;

            console.log(`Applying scale to ${partName}: Factor=${scaleFactor.toFixed(3)}, OrigSize=[${origSize[0].toFixed(1)}, ${origSize[1].toFixed(1)}], NewSize=[${newWidth.toFixed(1)}, ${newHeight.toFixed(1)}]`);

            // Update current part size if changed significantly
            if (Math.abs(part.size[0] - newWidth) > 1e-3 || Math.abs(part.size[1] - newHeight) > 1e-3) {
                 part.size = [newWidth, newHeight];
                 scalingApplied = true;
            } else if (partName === 'body' && (part.size[0] !== origSize[0] || part.size[1] !== origSize[1])) {
                 // Ensure body is explicitly set back to original size if needed
                 part.size = [...origSize];
                 // Don't set scalingApplied=true if only body reset to original config size
            }
        });
        ...
        // 8. Update Display / Log / Render
         if(scalingApplied){
            const displayDiv = document.getElementById('scale-ratios-display');
            if (displayDiv) {
                // Display factors relative to original config size
                displayDiv.innerHTML = `
                    <strong>计算出的缩放因子 (相对Config尺寸):</strong><br>
                    车身 (Body): ${bodyScaleFactor.toFixed(3)}<br>
                    大臂 (Boom): ${boomScaleFactor.toFixed(3)}<br>
                    小臂 (Arm): ${armScaleFactor.toFixed(3)}<br>
                    铲斗 (Bucket): ${bucketScaleFactor.toFixed(3)}
                `;
            }
            updatePartsPositions();
            renderExcavator();
        ...
        console.groupEnd(); // Ensure group ends on error
    }
}
...
function updatePartMeasurements() {
    ...
    const realBodyHeightRef = parseFloat(document.getElementById('real-body-height')?.value);
    const realBodyToBoomDist = parseFloat(document.getElementById('real-body-boom')?.value);
    const realBoomToArmDist = parseFloat(document.getElementById('real-boom-arm')?.value);
    const realArmToBucketDist = parseFloat(document.getElementById('real-arm-bucket')?.value);
    const realBucketToTipDist = parseFloat(document.getElementById('real-bucket-tip')?.value);
    ...
    if (hasRealBodyBoomRef) {
        ...
        realRatioBodyBoom = 'N/A';
    }

    // Generate HTML
    // Use ORIGINAL Body->Boom pixel distance (based on config sizes) as the RENDER reference unit
    const findOrigConf = (partName) => originalConfigData.find(p => p.part === partName);
    const origBodyConf = findOrigConf('body');
    // Calculate reference using original config size (half-real)
    ...
    // Header Info
    let html = `<div style="font-weight: bold; margin-bottom: 5px;">渲染参考: Config 车身->大臂 距离 = ${renderRefPx > 0 ? renderRefPx.toFixed(1) + 'px' : 'N/A'}</div>`;
    ...
    if (!isNaN(realBodyHeightRef) && realBodyHeightRef > 0) {
         html += `<div style="font-weight: bold; margin-bottom: 5px; font-size: 0.9em; color: #555;">(信息: 真实车身参考高: ${realBodyHeightRef.toFixed(0)} mm)</div>`;
    }
    ...
    renderOrder.forEach((partName) => {
        ...
        if (!part || !origPartConf) { html += `<tr><td>${partName}</td><td colspan="3">部件数据丢失</td></tr>`; return; }

        // Calculate CURRENT rendered distance using CURRENT size and ORIGINAL anchors
        const currentPixelDistance = calculatePartDistanceUsingSize(origPartConf.rotate_anchor, origPartConf.next_anchor, part.size);
        ...
        // Determine which real ratio (relative to realBodyToBoomDist) to show for this segment
        let realRatioToShow = '---';
...
// calculatePartsRatio is not used by measurement display anymore
